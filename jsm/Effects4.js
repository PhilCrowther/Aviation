	...
	if (bmt_.MakFlg[n]) {
		if (!bmt_.SpcCnt[n]) {		// If Space Counter = 0;
			for (let t = 0; t < bmt_.Trails; t++) {
				...
				bmt_.SmkPos[n].y = bmt_.SmkSpd[n][t].y*bmt_.SmkTim[n]-0.5*GrvMPS*(bmt_.SmkTim[n]**2)+bmt_.SmkRnd[n][t].y*Math.random()+bmt_.SmkOff[n][t].y;
				bmt_.SmkPos[n].z = bmt_.SmkSpd[n][t].z*bmt_.SmkTim[n]+bmt_.SmkRnd[n][t].z*Math.random()+bmt_.SmkOff[n][t].z;
				bmt_.SmkSpr[n][t][bmt_.SmkIdx].position.copy(bmt_.SmkPos[n]);
