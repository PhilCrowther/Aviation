...
import {color,MeshStandardNodeMaterial} from 'three/tsl';
...
let GrdMap = function (grd_, scene, vertexShader, vDepth, cameraNear, cameraFar) {
...
//- Grid 2 ---------------------------------------------------------------------
	...
	init1GrMap(grd_.Grx[0], grd_, scene, vertexShader, vDepth, cameraNear, cameraFar);
	init1GrMap(grd_.Grx[1], grd_, scene, vertexShader, vDepth, cameraNear, cameraFar);
	init1GrMap(grd_.Grx[2], grd_, scene, vertexShader, vDepth, cameraNear, cameraFar);
}
...
function init1GrMap(grx_, grd_, scene, vertexShader, vDepth, cameraNear, cameraFar) {
	// Load Variables
	...
	let material = new MeshStandardNodeMaterial({colorNode: color(0xc00000)});
		material.vertexNode = vertexShader(vertexShaderParams); // ### log
		material.depthNode = perspectiveDepthToLogarithmicDepth(vDepth, cameraNear, cameraFar); // ### log
