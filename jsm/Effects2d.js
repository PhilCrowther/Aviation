	...
	for (let n = 0; n < aaf_.ObjNum; n ++) {
		...
		for (let i = 0; i < aaf_.AAANum; i ++) {
			...
			if (
				...
				if (!aaf_.SmkDTm[n] && aaf_.AAAFlg[n]) { // If Smoke Delay = 0 and still firing
		...
		//	Smoke Timer (Creates Delay That Skips Several Bullets)
...
//		if (gen_.SndFlg && aaf_.SmkFlg[n]) aaf_.SndPtr[n].play();
		...
		if (aaf_.SmkFlg[n]) { // Compute Delay and Start Countdown
			let X = aaf_.ExpGrp[n].position.x; // SndMsh attached to SmkPtr
