...
//	temp	this.air_.AGBank = MaxVal(this.air_.AGBank,this.air_.BnkMax);		// Max values	
	...
	if (this.air_.GrdFlg) {
		...
		if (this.dat_.TDrAng) {
			...
			if ((this.air_.SpdKPH >= this.dat_.TDrSpd) && this.AuFlag) {	//211031
				this.AuFlag = 0;	// Tail Up on Take-Off Roll
				this.air_.CfLift = (this.dat_.AngInc+this.air_.FlpPct*this.dat_.FlpAIn)/10;
			}
		}
		else if (this. air_.ACPAdj < 0) this. air_.ACPAdj = 0; // Never negative while on ground
	}
