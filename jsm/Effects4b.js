...
let bmsOpaVal = array([0.2,0.6,1.0]);
let Global = uniform(1,0);
...
function moveExpBom(bom_,bmx_,bmt_,bms_,air_,gen_,tim_,n) {
	...
	if (gen_.SndFlg && bom_.SndFlg[n]) {
		...
		bom_.SndPtr[n].play();
		bom_.SndFlg[n] = 0;
//		bmsOpaVal.value[n] = bmsOpaBeg[n]; // Init Smoke Opacity
	}
...
let	updateGlobal = Fn((n) => {
    Global = bmsOpaVal.element(n);
});

//smokeNodeMaterial.opacityNode = Fn((n) => {
//	const Global = bmsOpaVal.element(n);
//	return materialOpacity.mul(targetOpacity);
//})();
