...
//	let MapPos = new Vector3();
	...
	for (let n = 0; n < aag_.ObjNum; n ++) {
		// Combined Rotation and Map Position of Parent plus Gun
//		MapRot.copy(aag_.XSHRot[n]).add(aag_.GunRot[n]); // XSHRot has value only for Ship
//		MapPos.copy(aag_.XSHPos[n]).add(aag_.GunPos[n]); // XSHPos has value only for Ship
		//	Gun Object Rotation (for show only)
		aag_.GunPtr[n].rotation.x = aag_.GunRot.x*DegRad; // Latitude
		aag_.GunPtr[n].rotation.y = aag_.GunRot.y*DegRad; // Longitude
...
//		MapPos.copy(aag_.XSHPos[n]).add(aag_.GunPos[n]); // XSHPos has value only for Ship
		...
		for (let i = 0; i < aag_.AAANum; i ++) {
			...
			// AAATim = time in flight (reset to zero at end); AAASp2 = delay (reset to zero when time passed)
				// Set Initial Rotation
				aag_.AAAPtr[n][i].rotation.x = aag_.GunRot.x*DegRad; // Latitude
				aag_.AAAPtr[n][i].rotation.y = aag_.GunRot.y*DegRad; // Longitude
				// Initial Map Position
				aag_.AAAMpP[n][i].copy(MapPos);
				// Set Initial Speed
				AAASV3 = new Spherical(AAASpT,(90-aag_.GunRot.x)*DegRad,Mod360(-aag_.GunRot.y)*DegRad);
