...
constructor(air_) {
	...
	if (this.air_.GrdFlg) {		// If on Ground
		this.air_.PwrPct = 0;	// Also specified in main program
		// this.air_.SpdKPH = 0; // Not true if on mvoing platform	
	...
	if (!this.air_.GrdFlg) {		// If Not on Ground
...
update() {
	...
	if (!this.air_.GrdFlg) this.air_.RotDif.x = ACLftD; // Pitch Degrees (before Gravity) If GrdFlg allow value to pass thru
	...
	if (!this.air_.GrdFlg) GrvACD = (GrvACP/this.air_.SpdMPF)*this.RadDeg;	// Degrees = (GrvACP/V)*(180/(PI()) = (GrvACP/V)*this.RadDeg
	...
	// d. DETERMINE IF GRDFLG OVERRIDES COMPS ..................................
	...
	if (!this.air_.GrdFlg && this.air_.MapPos.y < this.air_.GrdZed+10) { // If close to ground, check ...
		...
		if (this.air_.MapPos.y <= Flor) {
			...
			// this.air_.AirRot.x = 0; // Set Default Pitch (if on Moving Platform, program will override)
			this.air_.AirRot.z = 0; // Set Default Bank (no override from Moving Platform yet)
			// this.air_.ACPAdj = 0; // Allow for Flared Landing
		}
	}
	...
	if (this.air_.GrdFlg) {
		...
		// this.air_.RotDif.x = 0; // Allow value to pass through 
		this.air_.RotDif.z = 0; // No user change to Bank - Both Wheels on ground
		// this.air_.AirRot.x = 0;	// On ground direction of flight is zero (not if on moving platform)
		// All Movement Done by ACPAdj since direction of Flight = 0;
		if (this.dat_.TDrAng) { // Taildragger
			if (this.air_.ACPAdj > this.dat_.TDrAng) this.air_.ACPAdj = this.dat_.TDrAng; // Limit ACPAdj	
