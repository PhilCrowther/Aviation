	...
	if (this.air_.GrdFlg) {
		...
		if (this.dat_.TDrAng) { // If a Tail Dragger
			...
			// At MinSpd set this.air_.CfLift so that this.air_.ACPAdj = 0.
				let TDrAng = this.dat_.TDrAng-(this.dat_.TDrAng*this.air_.SpdKPH/this.dat_.TDrSpd);
				if (TDrAng < this.air_.ACPAdj) this.air_.ACPAdj = TDrAng; // prevent tail from popping up
			}
