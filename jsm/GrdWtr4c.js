...
_initGeoMat(grd_,scene) {
	...
	if (this.grd_.Src[0]) {
		...
		this.grd_.LdI.load(this.grd_.Src[0], function(image) { // Load, Split and Save Textures
			this.grd_.Ctx.drawImage(image,0,0,ImgSiz,ImgSiz);
			let idx = 0;
			let siz = ImgSiz/4;
			for (let z = 0; z < 4; z++) {
				for (let x = 0; x < 4; x++) {
					let ImgDat = this.grd_.Ctx.getImageData(siz*x, siz*z, siz, siz);
					let texture = new DataTexture(ImgDat.data, siz, siz);
					texture.format = RGBAFormat;
					texture.magFilter = LinearFilter;
					texture.minFilter = LinearMipMapLinearFilter;
					texture.generateMipmaps = true;
					texture.needsUpdate = true;
					this.grd_.Df0[idx] = texture;
					idx++;
				}
			}
		});
		// Grid2 - Static Color Map Texture
		this.grd_.LdT.load(this.grd_.Src[0], function(texture) {	// Load and Save entire Texture
			texture.format = RGBAFormat;
			texture.magFilter = LinearFilter;
			texture.minFilter = LinearMipMapLinearFilter;
			texture.generateMipmaps = true;
			...
			texture.needsUpdate = true;
			this.grd_.Dif = texture;
		});
	}
	// Load Roughness Texture
	if (this.grd_.Src[1]) {
		// Grid0 and Grid1 - Static Color Texture - Divided into 4 Parts
		this.grd_.LdI.load(this.grd_.Src[1], function(image) { // Load, Split and Save Textures
			this.grd_.Ctx.drawImage(image,0,0,ImgSiz,ImgSiz);
			let idx = 0;
			let siz = ImgSiz/4;
			for (let z = 0; z < 4; z++) {
				for (let x = 0; x < 4; x++) {
					...
					texture.generateMipmaps = true;
					texture.needsUpdate = true;
					this.grd_.Rf0[idx] = texture;
					idx++;
				}
			}
		});
		// Grid2 - Static Color Map Texture
		this.grd_.LdT.load(this.grd_.Src[1], function(texture) {	// Load and Save entire Texture
			texture.format = RGBAFormat;
			texture.magFilter = LinearFilter;
			texture.minFilter = LinearMipMapLinearFilter;
			texture.generateMipmaps = true;
			...
			texture.offset.set(0,0);
			texture.needsUpdate = true;
			this.grd_.Ruf = texture;
		});
	}
	// Grid2 - Static Normal Map
	if (this.grd_.Src[2]) {
		this.grd_.LdT.load(this.grd_.Src[2], function(texture) {
			texture.format = RGBAFormat;
			texture.magFilter = LinearFilter;
			texture.minFilter = LinearMipMapLinearFilter;
			texture.generateMipmaps = true;
			texture.wrapS = texture.wrapT = RepeatWrapping;
			texture.offset.set(0,0);
			texture.repeat.set(this.grd_.Stp*this.grd_.Stp/2,this.grd_.Stp*this.grd_.Stp/2);
			texture.needsUpdate = true;
			this.grd_.Gr2 = texture;
		});
	}
