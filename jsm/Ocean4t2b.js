...
//	this.Spd = wav_.Spd;
	...
	//= Render =================================================================
	await this.renderer.computeAsync(this.initSpectrumComp);
	await this.renderer.computeAsync(this.butterflyComp,[1,8,1]);
	// Static Targets
...
update() {
	// 2. Initial
	if (this.initPhase) {
		await this.renderer.computeAsync(this.pingPhaseComp);
		this.initPhase = false;
	}
	else {
		await this.renderer.computeAsync(this.pingPhase ? this.pingPhaseComp : this.pongPhaseComp);
	}
	await this.renderer.computeAsync(this.pingPhase ? this.pongPhaseComp : this.pingPhaseComp);	
	this.pingPhase = !this.pingPhase;
	// 3. New Spectrum from PingPhase or PongPhase
	await this.renderer.computeAsync(this.pingPhase ? this.pingSpectrumComp : this.pongSpectrumComp);
	...
	for (let i = 0; i < iterations; i++) {	// Horizontal Ping/Pong
		pingPong = !pingPong;
		this.stepBF.value = i;
		if (i == 0) await this.renderer.computeAsync(this.initDspHrzComp);	// if first rep, then New Spectrum to PingHrz
		else {	// Otherwise, Ping/Pong
			await this.renderer.computeAsync(pingPong ? this.pingDspHrzComp : this.pongDspHrzComp);
		}
	}
	for (let i = 0; i < iterations; i++) {	// Vertical Ping/Pong
		pingPong = !pingPong;
		this.stepBF.value = i;
		await this.renderer.computeAsync(pingPong ? this.pingDspVrtComp : this.pongDspVrtComp);	// Ping/Pong
	}
	// 5. Displacement
	await this.renderer.computeAsync(this.permutationComp);
	await this.renderer.computeAsync(this.compNormalComp);	
};	// End of Update
