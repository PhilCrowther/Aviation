...
//				xag_.BulMpS[n][i].x = -xag_.BulMpS[n][i].x; // This fixes direction, but not rotation
				xag_.BulMpS[n][i].y = BulSpT * Math.sin(xag_.XACRot[n].x*DegRad);
				let GS = BulSpT * Math.cos(xag_.XACRot[n].x);
//				let YR = Mod360((xag_.XACRot[n].y+90)*DegRad; // X is backwards
				let YR = Mod360(-xag_.XACRot[n].y)*DegRad; // 
				xag_.BulMpS[n][i].z = GS * Math.sin(YR);
				xag_.BulMpS[n][i].x = GS * Math.cos(YR);
				//
