...
function initAAGuns(aag_,air_,gen_) {
	...
	for (let n = 0; n < aaf_.ObjNum; n ++) {
		...
		aag_.SndDTm[n] = 0;
		// Bullets - Sub-arrays			
		aag_.AAAPtr[n] = Array.from({length: aag_.AAANum},() => []); // Bullet Objects
		aag_.AAAMpS[n] = Array.from({length: aag_.AAANum},() => []); // Bullet Map Speed (Vector3)
		aag_.AAAMpP[n] = Array.from({length: aag_.AAANum},() => []); // Bullet Map Position (Vector3)	
		aag_.AAATim[n] = Array.from({length: aag_.AAANum},() => []); // Bullet Time in flight
	}
