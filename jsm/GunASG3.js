...
function moveAAGuns(aag_,air_,AltDif,DLTime,GrvDLT,SndFlg) {
	...
	for (let n = 0; n < aag_.ObjNum; n ++) {
		...
		if (aag_.SmkPtr[n].visible = true) {
			...
			// Reduce Opacity
			aag_.SmkMat[n].opacityNode = aag_.SmkMat[n].opacityNode - aag_.SmkOpR;
			if (aag_.SmkMat[n].opacityNode < 0) aag_.SmkMat[n].opacityNode = 0;
		}
		// Smoke Timer
...
function makMsh() {
	...
	let material = new MeshBasicNodeMaterial();
		material.colorNode = color("black");
		material.transparent = true;
		material.opacityNode = 0;
