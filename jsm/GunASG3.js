...
function loadXSHBul(xsg_,scene) {
	...
	for (let n = 0; n < xsg_.ObjNum; n ++) {
		// Gun Object
//		xsg_.GunPtr[n].rotation.copy(xsg_.GunRot[n]);
//		xsg_.GunPtr[n].position.copy(xsg_.GunPos[n]);
//		scene.add(xsg_.ObjAdr[n]);
		// Load Bullets
...
//		xsg_.GunPtr[n].position.x = xsg_.MapPos[n].x-air_.MapPos.x;
//		xsg_.GunPtr[n].position.y = xsg_.MapPos[n].y-AltDif;
//		xsg_.GunPtr[n].position.z = air_.MapPos.z-xsg_.MapPos[n].z;
		...
		for (let i = 0; i < xsg_.AAANum; i ++) {
			...
			// AAATim = time in flight (reset to zero at end); AAASp2 = delay (reset to zero when time passed)
				...
				// Initial Map Position
				xsg_.AAAMpP[n][i].copy(xsg_.GunRef[n].position);
				// Set Initial Speed
