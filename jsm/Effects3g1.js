...
import {color,mix,positionLocal,range,rotateUV,texture,time,uniform,uv,uniformArray,float,array} from 'three/tsl';
...
//let bsmOpacity = uniformArray([0.6,0.6,1.0],'float');
let bsmOpacity = array([ float(0.6),float(0.6),float(0.6)]).toVar('bsmOpacity');
...
//	let opacityNode = textureNode.a.mul(life.oneMinus()).mul(bsmOpacity.element(n).assign(bsmOpaVal(n))); //
	let opacityNode = textureNode.a.mul(life.oneMinus()).mul(bsmOpacity.element(n)); //
...
function moveBomSmk(bms_,bom_,gen_,n) {
	...
	if (!bms_.GroFlg[n]) {
		...
		if (bms_.RemSiz[n] < 0.001) {
			...
			bsmOpacity.element(n).assign(bsmOpaVal[n])
		}
	}
	//	Resize
