...
//		let offsetRange = range(new THREE.Vector3(-2,3,-2), new THREE.Vector3(2,5,2));
		...
		//-	Smoke
		grf_.SmkMat[n] = new THREE.SpriteNodeMaterial();
		...
		grf_.SmkMat[n].positionNode = range(new THREE.Vector3(-2,3,-2), new THREE.Vector3(2,5,2)).mul(lifeTime);
		grf_.SmkMat[n].scaleNode = scaleRange.mul(lifeTime.max(0.3));
		grf_.SmkMat[n].depthWrite = false;
		grf_.SmkMsh[n] = new THREE.Mesh(new THREE.PlaneGeometry(1,1),grf_.SmkMat[n]);
		grf_.SmkMsh[n].scale.setScalar(grf_.ObjSiz);
		grf_.SmkMsh[n].count = 2000;
		grf_.SmkMsh[n].position.y = 100;
		//- Fire
		grf_.FyrMat[n] = new THREE.SpriteNodeMaterial();
		grf_.FyrMat[n].colorNode = mix(color(0xb72f17),color(0xb72f17),life);
		grf_.FyrMat[n].positionNode = range(new THREE.Vector3(-1,1,-1),new THREE.Vector3(1,2,1)).mul(lifeTime);
		grf_.FyrMat[n].scaleNode = grf_.SmkMat[n].scaleNode;
		grf_.FyrMat[n].opacityNode = opacityNode.mul(.5);
		grf_.FyrMat[n].blending = THREE.AdditiveBlending;
		grf_.FyrMat[n].transparent = true;
		grf_.FyrMat[n].depthWrite = false;
		grf_.FyrMsh[n] = new THREE.Mesh(new THREE.PlaneGeometry(1,1),grf_.FyrMat[n]);
		grf_.FyrMsh[n].scale.setScalar(grf_.ObjSiz);
		grf_.FyrMsh[n].count = 1000;
		grf_.FyrMsh[n].position.y = -100;
...
function initAirFyr(xaf_) {
	for (let n = 0; n < xaf_.ObjNum; n ++) {
		...
		xaf_.SmkMat[n].positionNode = range(new THREE.Vector3(-.5,3,-.5), new THREE.Vector3(1,5,1)).mul(lifeTime);
		...
		xaf_.FyrMat[n].positionNode = range(new THREE.Vector3(-.25,1,-.25),new THREE.Vector3(.5,2,.5)).mul(lifeTime);
