...
function initAAAGun(aaf_,air_,gen_) {
	...
	for (let n = 0; n < aaf_.ObjNum; n ++) {
		...
		aaf_.ExpGrp[n].add(aaf_.SmkPtr[n]);	// Add to Group
		aaf_.SmkPtr[n].visible = false;		// hide it
		//	Explosion Center
		aaf_.ExpPtr[n] = makeSphere("crimson");
		aaf_.ExpGrp[n].add(aaf_.ExpPtr[n]);	// Add to Group
	} // end of n
}
	...
	for (let n = 0; n < aaf_.ObjNum; n ++) {
		...
		for (let i = 0; i < aaf_.AAANum; i ++) {
			...
			if (
				...
				if (aaf_.SmkPtr[n].visible = false && aaf_.AAAFlg[n]) { // Smoke Delay = 0 and still firing
...
//					aaf_.SmkDTm[n] = aaf_.SmkDMx[n]; // Reset Delay Timer
		...
		if (aaf_.SmkPtr[n].visible = true) {
			aaf_.SmkGrp[n].position.x = aaf_.SmkMpP[n].x - air_.MapPos.x;
			aaf_.SmkGrp[n].position.y = aaf_.SmkMpP[n].y - air_.MapPos.y;
			aaf_.SmkGrp[n].position.z = air_.MapPos.z - aaf_.SmkMpP[n].z;
			aaf_.SmkMat[n].rotation = Mod360((air_.AirRot.z + aaf_.SmkRot[n])) * DegRad;
			// Reduce Opacity (this acts as timer)
...
//		if (aaf_.SmkDTm[n] < 0) aaf_.SmkDTm[n] = 0;
		//	Explosion (Red Flash)
		if (aaf_.SmkFlg[n]) {		// Reset with Each Frame
...
//		if (gen_.SndFlg && aaf_.SmkFlg[n]) aaf_.SndPtr[n].play();
		...
		if (aaf_.SmkFlg[n]) { // Compute Delay and Start Countdown
			let X = aaf_.SmkGrp[n].position.x; // SndMsh attached to SmkPtr
			let Z = aaf_.SmkGrp[n].position.z;
			let delay = (Math.sqrt(X*X+Z*Z)/343); // In Seconds
