...
update() {
	...
	if (!this.air_.GrdFlg && this.air_.MapPos.y < this.air_.GrdZed+10) { // If close to ground, check ...
		...
		if (this.air_.MapPos.y <= Flor) {
			...
			this.air_.AirRot.z = 0; // Set Default Bank (if on Moving Platform, program will override)
	...
	if (this.air_.GrdFlg) {
		...
		this.air_.RotDif.z = -this.air_.AirRot.z; // No user change to Bank - Both Wheels on ground
		this.air_.RotDif.x = -this.air_.AirRot.x; // No user change to Pitch 
