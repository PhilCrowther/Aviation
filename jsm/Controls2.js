...
function initCamera(cam_,air_,key_,gen_,mxr_,vxr_) {
	...
	//- Final Adjustments ------------------------------------------------------
	cam_.CamPar.add(cam_.MshRot); 			// Attach Rotator (Switch Camera Location)	
...
function moveCamera(cam_,air_,key_,gen_) {
	...
	// View Keys (NumLock)
		...
		if (cam_.LBkflg) cam_.CamLLD.y = 135;	// Look Left 135
		if (cam_.RBkflg) cam_.CamLLD.y = 225;	// Look Right 135
	}
