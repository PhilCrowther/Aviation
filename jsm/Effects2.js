...
function loadAAAGun(aag_,gen_,anm_) {
	if (aag_.GunSrc) {			// Procees only if there is an Gun Object file
...
function initAAGuns(aag_,air_,gen_,anm_) {
	...
	for (let n = 0; n < aag_.ObjNum; n ++) {
		...
		if (aag_.GunPtr[n]) {
			...
			aag_.GunPtr[n].position.y = MapPos.y-aag_.GunPos.y-gen_.AltDif;
...
//			if (aag_.ActLon[n]) aag_.ActLon[n].setTime(aag_.AnmLon[n]/anm_.anmfps);
//			if (aag_.ActLat[n]) aag_.ActLat[n].setTime(aag_.AnmLat[n]/anm_.anmfps);
			if (aag_.ActLon[n]) aag_.ActLon[n].setTime(aag_.AnmLon[n]/24);
			if (aag_.ActLat[n]) aag_.ActLat[n].setTime(aag_.AnmLat[n]/24);
		}
		// Targeting
		if (aag_.GunTar) {
