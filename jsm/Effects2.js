...
function initXSHGun(xsg_,gen_) {
	...
	for (let n = 0; n < xsg_.ObjNum; n ++) {
		...
		xsg_.SmkMat[n].opacity = 0.0;		// prevent black square from appearing in front of aircraft [260504]
...
function moveXSHGun(xsg_,xsh_,gen_,tim_) {
	for (let n = 0; n < xsg_.ObjNum; n ++) {
		...
		// Smoke Dealy
		if (xsg_.SmkTim[n]) {
			xsg_.SmkMat[n].opacity = xsg_.SmkMat[n].opacity - xsg_.SmkOpR;
			if (xsg_.SmkMat[n].opacity < 0) {
				xsg_.SmkMat[n].opacity = 0;
			}
			xsg_.SmkTim[n] = xsg_.SmkTim[n] - tim_.DLTime;
			if (xsg_.SmkTim[n]) < 0) {
				xsg_.SmkTim[n] = 0;
				xsg_.SmkPtr[n].visible = false;
			}
