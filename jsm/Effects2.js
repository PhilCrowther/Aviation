	...
	for (let n = 0; n < aaf_.ObjNum; n ++) {
		gen_.gltfLd.load(aaf_.GunSrc, function (gltf) { // The OnLoad function
			...
			gen_.audoLd.load(aaf_.FirSrc, function(buffer) {
				...
				init1Sound(aaf_.FirPtr[n],RefDst,aaf_.SndVol,1,0,aaf_.FirMsh[n]);
...
//	}
	...
	for (let n = 0; n < aaf_.ObjNum; n ++) {
		...
		if (aaf_.FirFlg[n]) { // Compute Delay and Start Countdown
			let X = aaf_.GunPos[n].position.x; // SndMsh attached to SmkPtr
			let Z = aaf_.GunPos[n].position.z;
	...
	for (let n = 0; n < aaf_.ObjNum; n ++) {
		...
		for (let i = 0; i < aaf_.AAANum; i ++) {
			...
			if (
				...
				if (!aaf_.SmkDTm[n] && aaf_.AAAFlg[n]) { // Smoke Delay = 0 and still firing [### 260513]
