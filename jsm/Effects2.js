...
function loadAAAGun(aag_,gen_) {
	if (aag_.ObjSrc) {
		for (let n = 0; n < aag_.ObjNum; n ++) {
			gen_.gltfLd.load(aag_.ObjSrc, function (gltf) { // The OnLoad function
				aag_.ObjAdr[n] = gltf.scene;
				aag_.ObjAdr[n].rotation.order = "YXZ";
				// Loasd Animations
				// Rotator
				let clip = THREE.AnimationClip.findByName(gltf.animations,"rotatorAction");
				aag_.ActLon[n] = new THREE.AnimationMixer(gltf.scene);
				let actun = aag_.ActLon[n].clipAction(clip);
				actun.play();
				if (aag_.ActLon[n]) aag_.ActLon[n].setTime(aag_.AnmLon[n]/anm_.anmfps);
				// Barrel
				clip = THREE.AnimationClip.findByName(gltf.animations,"barrelAction");
				aag_.ActLat[n] = new THREE.AnimationMixer(gltf.scene);
				actun = aag_.ActLat[n].clipAction(clip);
				actun.play();
				if (aag_.ActLat[n]) aag_.ActLat[n].setTime(aag_.AnmLat[n]/anm_.anmfps);
				// Initialize
				aag_.ObjAdr[n].position.y = -1000; // Temporary Position (aaf value not initialized yet)
				gen_.scene.add(aag_.ObjAdr[n]);
			});	
		}
	}
}
...
function initAAAGun(aag_,txt_,air_,gen_,anm_) {
	if (aag_.ObjNum) {
		aag_.SmkMap = txt_.ObjTxt[aag_.SmkMap];
		initAAGuns(aag_,air_,gen_,anm_);
...
function moveAAAGun(aag_,air_,gen_,tim_,anm_) {
	moveAAGuns(aag_,air_,gen_,tim_,anm_);
	// Explosion
	for (let n = 0; n < aag_.ObjNum; n ++) {
		if (aag_.SmkFlg[n]) {
...
function initAAGuns(aag_,air_,gen_,anm_) {
	//- Standard Values
	for (let n = 0; n < aag_.ObjNum; n ++) {
...
function moveAAGuns(aag_,air_,gen_,tim_,anm_) {
	//- Combined Rotation and Map Position of Parent plus Gun
	let MapRot = new Vector3();
	let MapPos = new Vector3();
