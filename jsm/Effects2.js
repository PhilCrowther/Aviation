...
function initAAGuns(aag_,air_,gen_) {
	...
	for (let n = 0; n < aag_.ObjNum; n ++) {
		// Combined Rotation and Map Position of Parent plus Gun
		MapPos.copy(aag_.GunPos[n]).add(aag_.ParPos); // Add Parent since bullets not linked
		MapRot.copy(aag_.GunRot[n]).add(aag_.ParRot); // Add Parent since bullets not linked
...
function moveAAGuns(aag_,air_,gen_,tim_) {
	...
	for (let n = 0; n < aag_.ObjNum; n ++) {
		...
		MapPos.copy(aag_.GunPos[n]).add(aag_.ParPos); // Add Parent since bullets not linked
		MapRot.copy(aag_.GunRot[n]).add(aag_.ParRot); // Add Paren since bullets not linked
