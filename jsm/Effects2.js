...
function initAAGuns(aag_,air_,gen_) {
	...
	for (let n = 0; n < aaf_.ObjNum; n ++) {
		...
		for (let i = 0; i < aag_.AAANum; i ++) {		
			...
			aag_.AAAPtr[n][i] = 0;	// Bullet Objects
			aag_.AAAMpS[n][i] = 0;	// Bullet Map Speed (Vector3)
			aag_.AAAMpP[n][i] = 0;	// Bullet Map Position (Vector3)	
			aag_.AAATim[n][i] = 0;	// Bullet Time in flight
		}	
	}
