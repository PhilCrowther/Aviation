...
function initVulkan(vlk_) {
	...
	let textureNode = texture(vlk_.ObjTxt[0], rotateUV(uv(),scaledTime.mul(rotateRange))); // r170
	...
	vlk_.ObjMat[0] = new SpriteNodeMaterial();
	vlk_.ObjMat[0].colorNode = mix(color("white"), smokeColor, life.mul(2.5).min(1)).mul(fakeLightEffect);
	vlk_.ObjMat[0].opacityNode = opacityNode;
	vlk_.ObjMat[0].positionNode = offsetRange.mul(lifeTime);
	vlk_.ObjMat[0].scaleNode = scaleRange.mul(lifeTime.max(0.3));
	vlk_.ObjMat[0].depthWrite = false;
	vlk_.ObjMat[0].transparent = true;
	vlk_.ObjAdr[0] = new Mesh(new PlaneGeometry(1, 1),vlk_.ObjMat[0]);
