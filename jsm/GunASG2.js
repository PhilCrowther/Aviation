	...
	for (let n = 0; n < aag_.ObjNum; n ++) {
		...
		for (let i = 0; i < aag_.AAANum; i ++) {
			...
			if (aag_.AAATim[n][i] > aag_.AAADLT) {
				aag_.AAATim[n][i] = 0;
				aag_.AAAPtr[n][i].visible = false;
				aag_.SndPtr[n].stop();	// Reset for next explosion // ### new
...
//					if (SndFlg && aag_.SndFlg[n]) aag_.SndPtr[n].play();
					if (SndFlg) {
						aag_.SndTim[n] = sndTim(aag_.AAAMpP[n][i]); // Set Delay
						aag_.SndFlg[n] = 1;
						console.log(aag_.SndTim[n]*60);
					}
				}
			}
			// Continue
...
//				aag_.SndPtr[n].stop();	// Reset for next explosion
			...
			aag_.SndTim[n] = aag_.SndTim-1; // Sound Delay
			if (aag_.SndTim[n] < 0) {
				aag_.SndTim = 0;
				if (SndFlg && aag_.SndFlg[n]) aag_.SndPtr[n].play();
				aag_.SndFlg[n] = 1;
...
//- Compute Delay ---------------------------------------------------------------
function sndTim(V3)
	let dist = Math.sqrt(V3.x*V3.x+V3.z*V3.z);
	let secs = dist/343;
	let frames = secs*60;
return frames;}
