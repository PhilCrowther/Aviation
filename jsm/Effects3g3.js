...
//	bms Global Opacity
let bmsOpaVal = [0.2,0.6,1.0];	// bms Global Opacity Values
...
function initBomSmk(bms_,bom_,n) {
	...
	let opacityNode = textureNode.a.mul(life.oneMinus()).mul(bmsGlobal.element(n)); //
...
function moveBomSmk(bms_,bom_,gen_,n) {
	...
	if (!bms_.GroFlg[n]) {
		...
		if (bms_.RemSiz[n] < 0.001) {
			...
			if (bmsOpaVal[n] < 0) bmsOpaVal[n] = 0.0;
			bmsGlobal.value = bmsOpaVal; // Updates the uniformArray
		}
	}
	//	Resize
