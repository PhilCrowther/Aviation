...
import {color,mix,positionLocal,range,rotateUV,texture,time,uniform,uv,uniformArray,float} from 'three/tsl';
...
let GlobalValue = uniformArray([0.2,0.6,1.0],'float');
...
//	let opacityNode = textureNode.a.mul(life.oneMinus()).mul(GlobalValue[n]); // OK
	let opacityNode = textureNode.a.mul(life.oneMinus()).mul(GlobalValue.element(n)); // OK
...
function moveBomSmk(bms_,bom_,gen_,n) {
	...
	//	Expand Quickly
	if (bms_.GroFlg[n]) {
		GlobalValue.element(n).value = 1.0;
...
//			GlobalValue.element(n).value = GlobalValue.element(n).value - 0.01;
			GlobalValue.value.element(n) = GlobalValue.value.element(n) - 0.01;
		}
	}
	//	Resize
