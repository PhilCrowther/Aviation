...
let initialSpectrumFS = `
	...
	precision highp int;
	#define 2_PI 6.283185307179586476
	#define G 9.81
	#define KM 370.0
	#define CM 0.23
	uniform vec2 u_wind;
	...
	void main(){
		...
		float S = (1.0/2_PI)*pow(k,-4.0)*(Bl+Bh)*(1.0+Delta*(2.0*cosPhi*cosPhi-1.0));
		float dk = 2_PI/u_grdsiz;
...
let phaseFS = `
	...
	void main(){
		...
		float n = (coordinates.x < u_grdres*0.5) ? coordinates.x : coordinates.x - u_grdres;
		float m = (coordinates.y < u_grdres*0.5) ? coordinates.y : coordinates.y - u_grdres;
		vec2 waveVector = (2_PI*vec2(n,m))/u_grdsiz;
		float phase = texture(u_phases,vUv).r;
		float deltaPhase = omega(length(waveVector))*u_deltaTime;
		phase = mod(phase+deltaPhase,2_PI);
...
let displacementHorizontalFS = `
	...
	void main(){	
		...
		float twiddleArgument = -2_PI*(index/u_subtransformSize);
